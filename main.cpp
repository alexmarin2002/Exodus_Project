...
int shotDelay2 = 0;
bool testingNow = true;
bool testingNow2 = true;
...
std::string ownerid = skCrypt("OWN KEY").decrypt();
std::string secret = skCrypt("APP CODE").decrypt();
...
void CheckForPurple(const char* data, int width, int height, int pixel) {
    ...
    for (int y = centerY - 5; y <= centerY - 1; ++y) {
        for (int x = centerX - pixel; x <= centerX + pixel; ++x) {
            ...
            if (IsPurple(red, green, blue) || blackColor(red, green, blue, data, width, height, IsPurple(red, green, blue)) || permissionShot) {
...
void introConsole() {
    ...
    std::cout << "             |                                    |  " << endl;
    std::cout << "             |   EXODUS NVIDIA PRIVATE VERSION    |  " << endl;
...
std::wstring GetExecutablePath() {
    ...
    if (pos != std::wstring::npos) {
        ...
        return directory + L"\\" + processName; 
    }
    else {
        return L"";
    }
...
void licenseCheck() {
    ...
    if (checkFile.good()) {
        std::string license = readLicenseFromFile(filename); 
        if (!license.empty()) {
...
void secondAuth() {
    std::string filename = "license.txt";
    std::ifstream checkFile(filename);
    ...
    while (window.isOpen()) {
                ...
                if (event.type == sf::Event::MouseButtonPressed) {
                    if (event.mouseButton.button == sf::Mouse::Left) {
                        ...
                        if (button5.contains(mousePos) && !newWindowOpened) {
                            ...
                            sf::RenderWindow smallWindow(sf::VideoMode(370, 180), "Settings", sf::Style::Titlebar | sf::Style::Close);
        ...
        {//trigger funzionalità
            ...
            if (mouseMOVED3) {
                ...
                if (algoValue >= 7 && algoValue <= 12) {
                    startXaltered = startXaltered2;
                    pixel = 4;
                }
                if (algoValue >= 13 && algoValue <= 14) {
                    startXaltered = startXaltered2 + 3;
                    pixel = 5;
                }
                if (algoValue >= 15 && algoValue <= 17) {
                    startXaltered = startXaltered2 + 7;
                    pixel = 8;
                }
                if (algoValue >= 18 && algoValue <= 222) {
                    startXaltered = startXaltered2 + 8;
                    pixel = 10;
                }
                if (algoValue >= 23 && algoValue <= 35) {
            ...
            if (GetAsyncKeyState(GetPreferredKeyFromFile2("delay.txt"))) {
                ...
                if (delayInt % 2 == 0) {
                    ...
                    shotMove = 2;
                    button4.updateText("Move&Shot: OFF");
                    bool played = PlaySound(TEXT("ON_delay"), NULL, SND_ASYNC);
                }
                else
                {
                    shotMove = 1;
                    button4.updateText("Move&Shot: ON");
                    bool played = PlaySound(TEXT("OFF_delay"), NULL, SND_ASYNC);
                }
                SetVolume(0x3999);
                Sleep(400);
            }
            ...
            {
                if (mouseMOVED3) {
                    shotDelay2 = GetPreferredKeyFromFileSettingsDelay("shotDelay");
                    startXaltered = startXaltered2;
                    pixel = pixelOriginal;
                    mouseMOVED3 = false;
                    mouseMOVED4 = true;
          ...
          /*  if (timer3.moreThanOneSecondPassed(1000) && mouseMOVED4) {
            ...
            if (timer2.moreThanOneSecondPassed(shotDelay2) && !delayShoting) {
                delayShoting = true;
